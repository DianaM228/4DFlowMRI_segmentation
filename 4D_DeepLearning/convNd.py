        ...
        for _ in range(next_dim_len):
            if self.num_dims-1 > max_dims:
                ...
                conv_layer = self.conv_f(in_channels=self.in_channels,
                                            ...
                                            padding=self.padding[1:],
                                            padding_mode=self.padding_mode,
                                            groups=self.groups) ### Define 3D conv
            ...
            # Store the layer
            self.conv_layers.append(conv_layer) ### Build 4D conv with the staked 3D conv
    ...
    def forward(self, input):
        ...
        if not self.is_transposed:
            ...
            if self.rank==0:
                ...
                inputShape[2] += 2*self.padding[0] ### time padding
        ...
        (b, c_i) = tuple(input.shape[0:2]) ## b= batch c_i = channels
        size_i = tuple(input.shape[2:])   ## resolution 4D 
        size_k = self.kernel_size

        if not self.is_transposed:
            # Compute the size of the output tensor based on the zero padding
            size_o = tuple([math.floor((size_i[x] + 2 * padding[x] - size_k[x]) / self.stride[x] + 1) for x in range(len(size_i))]) ## limit sup
            # Compute size of the output without stride (without padding and stride 1)
        ...
        for i in range(size_k[0]): ### Kernel
            # iterate inputs first dimmension
            for j in range(size_i[0]): ### Time (padded frames) 

                # Add results to this output frame
                if self.is_transposed:
                    out_frame = i + j*self.stride[0] - self.padding[0]
                else: ## Positions for the output and kernel center
                    out_frame = j - (i - size_k[0] // 2) - (size_i[0] - size_ons[0]) // 2 - (1-size_k[0]%2) ### outIndext
                    k_center_position = out_frame % self.stride[0]
                    out_frame = math.floor(out_frame / self.stride[0])
                    if k_center_position != 0:
                        #print('no valid kernel center position',k_center_position)
                        continue
                        
                
                if out_frame < 0 or out_frame >= size_o[0]: ####### limits of convolution 
                    #print('Out of limit ',out_frame)
                    continue

                # Prepate input for next dimmension
                conv_input = input.view(b, c_i, size_i[0], -1) ## flat the spatial resolution e.g. input = [3, 1, 27, 34, 35, 36] then conv_input = [3, 1, 27, 42840]
                conv_input = conv_input[:, :, j, :].view((b, c_i) + size_i[1:])  ###### extract j temporal dim > conv_input = [3, 1, 34, 35, 36]
                #print('Frame ',j)
                # Convolve
                frame_conv = \
                ...
                if empty_frames[out_frame] is None:
                    ...
                    # print('llenando frame',out_frame)
                else:
                    frame_results[out_frame] += frame_conv
                    # print('sumando a frame',out_frame)
