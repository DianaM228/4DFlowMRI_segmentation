...
def save_images(input, target, pred, affine, count, batch_size, save_path,names):
    for i in range(batch_size):
        ...
        nib.save(current_input, os.path.join(save_path, 'InputTestNorm_' + str(count)+'_' + names[i]+'.gz' ))
        ...
        nib.save(current_target, os.path.join(save_path, 'TargetTest_' + str(count)+'_' + names[i]+'.gz'))
        # Assume that 0 is the background, 1 liver, 2 tumour
        ...
        nib.save(current_pred, os.path.join(save_path, 'PredTest_' + str(count)+'_' + names[i]+'.gz' ))
...
def val_test(val_test_loader, model, criterion, device, save_path=None, verbose=False, dgminfo=None):
    ...
    model.eval()
    print('VAAAAAAAAAAAAALLLLL VAAAAAAAAAAAAALLL')
    with torch.no_grad():
        end = time.time()
