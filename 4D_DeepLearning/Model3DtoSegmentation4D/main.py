...
def main():
    ...
    Path_images = natsorted(glob.glob(os.path.join(args.input_path,'Images','*')))   
    ...
    if args.test:
        ...
        #Path_images, _ = train_test_split(Path_images, test_size=0.166, random_state=42)
    ...
    if args.Evaluation_Method != None:
        if args.Evaluation_Method == 'personalized':
            ...
            for nf,f in enumerate(Index['Train']): ##### Fold in Train
                ...
                if model_to_train == 'UNet3DAntoine':
                    model = UNet3DAntoine(in_channel=1, out_channel=num_organs+1)  ## +1 = because of the background
                    print('UNet3DAntoine')
                ...
                train_dataset = AortaDataset(database_path=args.input_path, list_ids=train_id,path_all=Path_images, norm_value=norm_value,
                                                    transform=data_augmentation, num_organs=num_organs)
                train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
                val_dataset = AortaDataset(database_path=args.input_path, list_ids=val_id,path_all=Path_images, norm_value=norm_value,
        ...
        ### Validación cruzada o Leave-one-out    
            ...
            if args.test:
                test_dataset = AortaDataset(database_path=args.input_path, list_ids=test_id,path_all=path_id_test, norm_value=norm_value,
                                              transform=None, num_organs=num_organs)
            ...
            for epoch in range(nb_epochs):
                ...
                if current_val_dice>best_Metric:
                    ...
                    torch.save(checkpoint,os.path.join(Folddir,'BestcheckPoint.pth.tar'))
                    val_test(val_loader, model, criterion, device, save_path=path_save_Best, verbose=verbose)
        ...
        #train_images, test_images = train_test_split(Path_images, test_size=0.1, random_state=42)
        train_id, val_id = train_test_split(np.arange(len(Path_images)), test_size=0.1, random_state=42)

        print("Generalisation")
        train_dataset = AortaDataset(database_path=args.input_path, list_ids=train_id,path_all=Path_images, norm_value=norm_value,
                                           transform=data_augmentation, num_organs=num_organs)
        #dt_ejm=train_dataset[0]    
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
        
        test_dataset = AortaDataset(database_path=args.input_path, list_ids=val_id,path_all=Path_images, norm_value=norm_value,
                                          transform=None, num_organs=num_organs)
        test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
        ...
        for epoch in range(nb_epochs):
            print('Epoch ',epoch)
            # train
            train_loss, train_dice = train(train_loader, model, criterion, optimizer, epoch, device, verbose)
            # test
            test_loss, test_dice = val_test(test_loader, model, criterion, device, verbose=verbose)
      
            # log the loss + dice
            writer.add_scalars('loss_generalisation/training-testing_loss', {'training': train_loss,
                                                                             'testing': test_loss}, epoch)
            writer.add_scalars('dice_generalisation/training-testing_dice', {'training': train_dice,
                                                                             'testing': test_dice}, epoch)
            if args.lr_schedule:
               lr_scheduler(test_loss)
            if args.early_stop:
                early_stopping(test_loss)
                if early_stopping.early_stop:
                    val_test(test_loader, model, criterion, device, save_path=args.output_path, verbose=verbose)
                    break
            ## Save best model    
             
            if test_dice>best_Metric:
                best_Metric=test_dice
                
                checkpoint = {'Parameters': Parameters,# Experiment parameters
                              'state_dict': model.state_dict(),# Model layers and weights
                              'optimizer': optimizer.state_dict(),# Optimizer
                              'loss_train': train_loss,# Average loss history in training set each epoch
                              'loss_val': test_loss,# Average loss history in validation set each epoch
                              'Metric_train':train_dice,
                              'Metric_val':test_dice,
                              'Batch': args.batch_size,
                              'epoch':epoch,
                              'Metric':loss_name
                              }
                torch.save(checkpoint,os.path.join(args.output_path,'BestcheckPoint.pth.tar'))
        ...
        #
        # Where to save the model
        model_save_path = os.path.join(args.output_path, 'Last_State_dict_model_' + model_to_train + '.pth.tar')
        # Save
