...
clitkPath = '/work/le2i/di4567ma/clitk_private/Install/bin'

clitkMattesMutualInformation = os.path.normpath(os.path.join(clitkPath,'clitkMattesMutualInformation'))
clitkNormalizedMutualInformation = os.path.normpath(os.path.join(clitkPath,'clitkNormalizedMutualInformation'))
clitkNormalizedCorrelationCoefficient= os.path.normpath(os.path.join(clitkPath,'clitkNormalizedCorrelationCoefficient'))
clitkBinarizeImage = os.path.normpath(os.path.join(clitkPath,'clitkBinarizeImage'))
clitkImageArithm = os.path.normpath(os.path.join(clitkPath,'clitkImageArithm'))
clitkArgmaxImage = os.path.normpath(os.path.join(clitkPath,'clitkArgmaxImage'))
clitkImageConvert = os.path.normpath(os.path.join(clitkPath,'clitkImageConvert'))
clitkMorphoMath = os.path.normpath(os.path.join(clitkPath,'clitkMorphoMath'))
clitkAutoCrop = os.path.normpath(os.path.join(clitkPath,'clitkAutoCrop'))
clitkCropImage = os.path.normpath(os.path.join(clitkPath,'clitkCropImage'))
clitkImageStatistics = os.path.normpath(os.path.join(clitkPath,'clitkImageStatistics'))
clitkStructuralSimilarityIndex = os.path.normpath(os.path.join(clitkPath,'clitkStructuralSimilarityIndex'))
clitkMeanSquares = os.path.normpath(os.path.join(clitkPath,'clitkMeanSquares'))
...
def LabelSelection(path_reg, TargetGray,phi,Best_N_Masks,MetricS,MetricW=None):
    ...
    if MetricS == 'NCC':
        MetricSelection = clitkNormalizedCorrelationCoefficient
    elif MetricS == 'NMI':
        MetricSelection = clitkNormalizedMutualInformation
    elif MetricS == 'MI':
        MetricSelection = clitkMattesMutualInformation
    elif MetricS == 'MS':
        MetricSelection = clitkMeanSquares
    if MetricW == 'NCC':
        MetricWeight = clitkNormalizedCorrelationCoefficient
    elif MetricW == 'NMI':
        MetricWeight = clitkNormalizedMutualInformation
    elif MetricW == 'MI':
        MetricWeight = clitkMattesMutualInformation
    elif MetricW == 'MS':
        MetricWeight = clitkMeanSquares
...
def LabelSelection2(path_reg, TargetGray,phi,Best_N_Masks,MetricS=None,MetricW=None,Warped_masks=None):
    ...
    if MetricS == 'NCC':
        ...
        MetricSelection = clitkStructuralSimilarityIndex
    ...
    if MetricW:
        if MetricW == 'NCC':
            ...
            MetricWeight = clitkStructuralSimilarityIndex
    ...
    if MetricW == None or MetricS == MetricW:
        ...
        for i in natsorted(os.listdir(path_reg)): # All registrations for patient n
              ...
              if MetricS == 'SSIM':
                  temporal = os.path.join(up(up(Warped_masks)),'Temporal'+str(phi)) # Temporal folder to save croped images and SSI maps
                  ...
                  Lcitk = [clitkMorphoMath,'-i',os.path.join(Warped_masks,i),'-o',Dilate,'-t','1','-m','10']
                  subprocess.run(Lcitk)
                  # AutoCrop Dilated mask
                  AutoCrop = os.path.join(temporal,'CropMask'+i)
                  Lcitk = [clitkAutoCrop,'-i',Dilate,'-o',AutoCrop]
                  subprocess.run(Lcitk)
                  #Crop SSI Map
                  cropSSIM = os.path.join(temporal,'CropSSIMap'+i)
                  Lcitk = [clitkCropImage,'-i',NameMapSSI,'-o',cropSSIM,'--like',AutoCrop]
                  subprocess.run(Lcitk)
                  # Mean of croped SSI map
                  Lcitk = [clitkImageStatistics, '-i',cropSSIM]
                  miN = subprocess.Popen(Lcitk, stdout=subprocess.PIPE)
                  out = miN.communicate()[0]
    ...
    if MetricW == None or MetricS == MetricW:
         ...
         for i in natsorted(os.listdir(path_reg)):
             ...
             if MetricS == 'MI':
                 ...
                 Lcitk = [clitkMattesMutualInformation, '-i', TargetGray, '-j', regn]
                 miN = subprocess.Popen(Lcitk, stdout=subprocess.PIPE)
                 out = miN.communicate()[0]
                 
             if MetricS == 'NMI':
                 Lcitk = [clitkNormalizedMutualInformation, '-i', TargetGray, '-j', regn]
                 miN = subprocess.Popen(Lcitk, stdout=subprocess.PIPE)
                 out = miN.communicate()[0]
             
             else:
                 Lcitk = [clitkNormalizedCorrelationCoefficient, '-i', TargetGray, '-j', regn]
                 miN = subprocess.Popen(Lcitk, stdout=subprocess.PIPE)
                 out = miN.communicate()[0]
         ...
         MetricSel = np.array(metric)    
         ri = list(MetricSel/max(MetricSel))
         # Selestion considering Best_N_Masks
         if Best_N_Masks=='True':
             OPathSelAll = [pathsAll for _,pathsAll in sorted(zip(ri,pathsAll),reverse=True)]
             OPathSel = OPathSelAll[0:int(phi)]
             Osri = sorted(ri,reverse=True)[0:int(phi)]
        ...
        for i in natsorted(os.listdir(path_reg)):
            ...
            Lcitk = [clitkMattesMutualInformation, '-i', TargetGray, '-j', regn]
            miN = subprocess.Popen(Lcitk, stdout=subprocess.PIPE)
            outMI = miN.communicate()[0]
        
            Lcitk = [clitkNormalizedCorrelationCoefficient, '-i', TargetGray, '-j', regn]
            miN = subprocess.Popen(Lcitk, stdout=subprocess.PIPE)
            outNCC = miN.communicate()[0]
                                
            metricMI.append(abs(float((outMI.decode()).split()[-1])))
            metricNCC.append(abs(float((outNCC.decode()).split()[-1])))
        
        
        # Weights
        MetricSelMI = np.array(metricMI)
        MetricSelNCC = np.array(metricNCC)
        riMI = list(MetricSelMI/max(MetricSelMI))
        riNCC = list(MetricSelNCC/max(MetricSelNCC))
        
        # selection according to metric
        if MetricS == 'MI':
            if Best_N_Masks=='True':
                OPathSelAll = [pathsAll for _,pathsAll in sorted(zip(riMI,pathsAll),reverse=True)]
                OPathSel = OPathSelAll[0:int(phi)]
                sriMI = sorted(riMI,reverse=True)[0:int(phi)]
                sriNCCAll = [riNCC for _,riNCC in sorted(zip(riMI,riNCC),reverse=True)]
                sriNCC = sriNCCAll[0:int(phi)]
                Sel_Met_MIAll = [metricMI for _,metricMI in sorted(zip(riMI,metricMI),reverse=True)]
                Sel_Met_MI = Sel_Met_MIAll[0:int(phi)]
                Sel_Met_NCCAll = [metricNCC for _,metricNCC in sorted(zip(riMI,metricNCC),reverse=True)]
                Sel_Met_NCC = Sel_Met_NCCAll[0:int(phi)]
            else:
                OPathSel=[pathsAll[s] for s in range(len(riMI)) if riMI[s] >= float(phi)]
                sriMI = [riMI[s] for s in range(len(riMI)) if riMI[s] >= float(phi)]
                sriNCC = [riNCC[s] for s in range(len(riMI)) if riMI[s] >= float(phi)]
                Sel_Met_MI = [metricMI[s] for s in range(len(riMI)) if riMI[s] >= float(phi)]
                Sel_Met_NCC = [metricNCC[s] for s in range(len(riMI)) if riMI[s] >= float(phi)]
            
            
        else:
            # Selection with respect to NCC
            if Best_N_Masks=='True':
                OPathSelAll = [pathsAll for _,pathsAll in sorted(zip(riNCC,pathsAll),reverse=True)]
                OPathSel = OPathSelAll[0:int(phi)]
                sriNCC = sorted(riNCC,reverse=True)[0:int(phi)]
                sriMIAll = [riMI for _,riMI in sorted(zip(riNCC,riMI),reverse=True)]
                sriMI = sriMIAll[0:int(phi)]
                Sel_Met_MIAll = [metricMI for _,metricMI in sorted(zip(riNCC,metricMI),reverse=True)]
                Sel_Met_MI = Sel_Met_MIAll[0:int(phi)]
                Sel_Met_NCCAll = [metricNCC for _,metricNCC in sorted(zip(riNCC,metricNCC),reverse=True)]
                Sel_Met_NCC = Sel_Met_NCCAll[0:int(phi)]
            else:
            
                OPathSel=[pathsAll[s] for s in range(len(riNCC)) if riNCC[s] >= float(phi)]
                sriMI = [riMI[s] for s in range(len(riNCC)) if riNCC[s] >= float(phi)]
                sriNCC = [riNCC[s] for s in range(len(riNCC)) if riNCC[s] >= float(phi)]  
                Sel_Met_MI = [metricMI[s] for s in range(len(riNCC)) if riNCC[s] >= float(phi)]
                Sel_Met_NCC = [metricNCC[s] for s in range(len(riNCC)) if riNCC[s] >= float(phi)]
        
        # return parameter
        if MetricW == 'MI':
            Osri = sriMI
            OMetricSel = Sel_Met_MI
        else: 
            Osri = sriNCC
            OMetricSel = Sel_Met_NCC
              
        
    return OPathSel, Osri, OMetricSel


def SelectionPWMV(path_reg=None,path_seg=None,target=None,phi=None,Best_N_Masks=False,MetricS=None,Labels=None,outF=None):
                ...
                if globals()['TF{}'.format(tf)][it.multi_index]==1: # Selection
                    ...
                    for l in range(Labels+1):
                        ...
                        if not os.path.isfile(outImg):
                            ...
                            binLabl = [clitkBinarizeImage,'-i',inima,'-l',str(l),'-u',str(l),'-o',outImg]
                    ...
                    # Multiply weight form SSIM map with both binarized images
    ...
    Larg = [clitkArgmaxImage,'-i', inArg, '-o', AuMask]
    subprocess.run(Larg)
...
def MultiLabWeightedMajorityVoting(Paths_Selected_Masks,ri_scores,out_Path,numLab):
    ...
    TemOut=os.path.normpath(os.path.join(out_Path,'Temporal_Results'))
    ...
    for i,m in enumerate(natsorted(Paths_Selected_Masks)):  # Selested masks      
        for l in range(numLab+1): # labels into mask
            outImg =  os.path.normpath(os.path.join(TemOut,'SelectedImage'+str(i)+'_Lab'+str(l)+'.nii.gz'))
            binLabl = [clitkBinarizeImage,'-i',os.path.normpath(m),'-l',str(l),'-u',str(l),'-o',outImg]
            subprocess.run(binLabl)
            
            # Multiply thresholded-selected image (recpect to label l) by the respetive weght
            outBxW = os.path.normpath(os.path.join(TemOut,'MulxW_'+'SelectedImage'+str(i)+'_Lab'+str(l)+'.nii.gz'))
            mulImg = [clitkImageArithm,'-i',outImg, '-t', '1','-s',str(ri_scores[i]),'-o',outBxW,'-f']
    ...
    for mw in range(numLab+1): # Labels into masks + 1 for background
        if len(globals()['paths_Threshold_{}'.format(mw)])==1: # If selected masks = 1 sum process isn't necesary
            ...
            ln=[clitkImageArithm,'-i',outS,'-s',str(sumris),'-t','11','-o',os.path.join(TemOut,'l'+str(mw)+'.nii'),'-f']
            subprocess.run(ln)
            imagesArgM.append('-i')
            ...
            for n in range(len(natsorted(globals()['paths_Threshold_{}'.format(mw)]))-1):
                if n==0:
                    ...
                    sumM = [clitkImageArithm,'-i',ImS,'-j',ImS1,'-o',outS,'-t','0','-f']
                    subprocess.run(sumM)
                    ...
                    sumM = [clitkImageArithm,'-i',outS,'-j',ImS,'-o',outS1,'-t','0','-f']
                    subprocess.run(sumM)
                    outS = outS1
            ...
            ln=[clitkImageArithm,'-i',outS,'-s',str(sumris),'-t','11','-o',os.path.normpath(os.path.join(TemOut,'l'+str(mw)+'.nii')),'-f']
            subprocess.run(ln)
            imagesArgM.append('-i')
            imagesArgM.append(os.path.join(TemOut,'l'+str(mw)+'.nii'))
        
    # Argmax
    arg = [clitkArgmaxImage]
    outarg = ['-o',os.path.normpath(os.path.join(out_Path,'Result_Mask_WMV2.nii'))]
    inarg = arg + imagesArgM + outarg
    subprocess.run(inarg)        

    return os.path.normpath(os.path.join(out_Path,'Result_Mask_WMV2.nii')), os.path.normpath(TemOut)
...
def PatchWeighetMajorityVoting_NCC(P_Selected_Images,Target,out_Path,Num_Labels,kernel=[3,3,3],stride='1'):
    ...
    for i,im in enumerate(natsorted(P_Selected_Images)): 
        ...
        argNCCIm = [clitkNormalizedCorrelationCoefficient,'-i',im,'-j',Target,'-k',K,'-s',stride,'-o',outNCCim,'-f']
        subprocess.run(argNCCIm)
        
        ## Absolute value 
        outAbs = os.path.join(steps,'Abs_NCC_Im_SelIM_'+str(i)+'.nii')
        argAbs = [clitkImageArithm,'-i',outNCCim,'-t','5','-s','0','-o',outAbs]
    ...
    if len(list_NCCAbs)==1:
        ...
        for m in range(len(list_NCCAbs)-1):
            if m==0:
                ...
                Mx = [clitkImageArithm,'-i',list_NCCAbs[m],'-j',list_NCCAbs[m+1],'-o',maxIM1,'-t','3']
                subprocess.run(Mx)                
              
            else:
                 # Maximum between NCCimages
                maxIM=os.path.join(steps,'MAX_NCC'+str(m)+'.nii')
                Mx = [clitkImageArithm,'-i',maxIM1,'-j',list_NCCAbs[m+1],'-o',maxIM,'-t','3']
                subprocess.run(Mx)
                maxIM1 = maxIM
    ...
    for it,d in enumerate(natsorted(list_NCCAbs)):
        ...
        listDiv = [clitkImageArithm,'-i',d,'-j',maxIM,'-t','2','-o',outDiv]
    ...
    if len(listRiIm)==1:
        ...
        for sr in range(len(listRiIm)-1):
            if sr==0:
                ...
                sumM = [clitkImageArithm,'-i',listRiIm[sr],'-j',listRiIm[sr+1],'-o',sumRi1,'-t','0']
                subprocess.run(sumM)

            else:
                # Sum Maks
                sumRi=os.path.join(steps,'Sum_Ri'+str(sr)+'.nii')
                sumM = [clitkImageArithm,'-i',sumRi1,'-j',listRiIm[sr+1],'-o',sumRi,'-t','0']
                subprocess.run(sumM)
                sumRi1 = sumRi
    ...
    for ii,mm in enumerate(natsorted(pathSel_Seg)):  # Selested masks      
        for l in range(Num_Labels+1): # labels into mask
            outImg =  os.path.join(steps,'SelectedImage'+str(ii)+'_Lab'+str(l)+'.nii.gz')
            binLabl = [clitkBinarizeImage,'-i',mm,'-l',str(l),'-u',str(l),'-o',outImg]
            subprocess.run(binLabl)
            
            convManual = [clitkImageConvert, '-i',outImg, '-o', outImg, '-t', 'float']
            subprocess.run(convManual)
            
            # Multiply thresholded-selected image (recpect to label l) by the respetive weight
            outBxW = os.path.join(steps,'MulxWIm_'+'SelectedImage'+str(ii)+'_Lab'+str(l)+'.nii.gz')
            mulImg = [clitkImageArithm,'-i',outImg, '-t', '1','-j',listRiIm[ii],'-o',outBxW,'-f']
            subprocess.run(mulImg)
            globals()['paths_Threshold_{}'.format(l)].append(outBxW)
    ...
    for mw in range(Num_Labels+1): # Labels into masks + 1 for background
        if len(globals()['paths_Threshold_{}'.format(mw)])==1: # If selected masks = 1 sum process isn't necesary
            ...
            ln=[clitkImageArithm,'-i',outS,'-j',sumRi1,'-t','2','-o',os.path.join(steps,'l'+str(mw)+'.nii'),'-f']
            subprocess.run(ln)
            imagesArgM.append('-i')
            imagesArgM.append(os.path.join(steps,'l'+str(mw)+'.nii'))
