...
class NormToTensor(object):
    ...
    def __call__(self, sample):
        ...
        np_img = np_img.reshape(1,shape_np_img[0],shape_np_img[1],shape_np_img[2]) # Add axis
        torch_image = torch.from_numpy(np_img).float() # convert numoy array to tensor
        torch_image = torch_image.permute(0, 3, 2, 1) # invert shape, free axis, slices, ...
...
class AortaDataset(Dataset):
    ...
    def __init__(self, database_path, list_ids,path_id, norm_value, transform=None, num_organs=1):
        """
        Args:
        ...
        for i in self.list_ids:
            self.img_files.append(all_images[i])
            basname = os.path.splitext(os.path.basename(all_images[i]))[0]
            keyname,namestr = NumInString(basname)            
            self.mask_files.append(os.path.join(self.database_path,'Masks', basname[0:namestr]+keyname+'MASK.nii.gz'))
            self.Images_Names.append(basname[0:namestr]+keyname+'MASK.nii.gz')
